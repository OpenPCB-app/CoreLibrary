...
its footprint. Every placement tool (`pack`, `audit:3d`, the contact sheet, the
...
(`bun audit:3d`, run as a release gate in CI). Mount type comes from the
...
| xy-centre   | model XY centre ≈ footprint centre (pads ∪ silk), tol `max(0.4, 0.3·extent)` |
...
1. Import / place the STEP; keep the sidecar transform **identity** unless the
   STEP is non-KLC.
2. `bun pack --version 0.0.0-dev --write-bounds` → regenerates GLBs, writes
   `boundsMm` / `boundsMinMm` / `boundsMaxMm` into the sidecar.
3. `bun audit:3d --release` → must be green (no error findings).
